...
| [Newtonsoft.Json](https://github.com/JamesNK/Newtonsoft.Json) | MIT (`LICENSE.md`, "Copyright (c) 2007 James Newton-King") | 2026-09-25, the file in the NuGet cache's 13.0.3 package. The mod ships the 11.0.1 copy bundled in MultiClient.Net; re-read that version's notice before the first release | Shipped next to MultiClient.Net (its netstandard2.0 build). Not read, not patched. The notice goes with the DLL (`apimplementation.md`, Next) |
